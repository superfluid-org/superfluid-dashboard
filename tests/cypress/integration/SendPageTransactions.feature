...
Feature: Stream transactional test cases
  ...
  Scenario: Creating a new stream
    ...
    # The successful send resets the form behind the dialog, which must clear the
    # buffer warning too (it used to stay latched on with a blank amount).
    And The buffer warning is not shown
    ...
    And The buffer warning is shown
  ...
  Scenario: Cancelling a stream
    ...
    # The successful cancel resets the form, clearing the buffer warning with it.
    And The buffer warning is not shown
    And User opens the transaction drawer
