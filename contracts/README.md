...
- **Fees:** a relay fee of `BASE_FEE_AMOUNT * feeUnits` is charged in `FEE_SUPER_TOKEN` to `FEE_RECEIVER` on every action: **1x** for any plain action and for cancelling a schedule, and a **new** schedule additionally pays **2x per keeper execution it reserves**, so it costs **3x** (start-only or stop-only) or **5x** (start and stop); **modifying** an existing schedule is **1x** (the keeper executions were reserved when it was first created, so the fee is not re-applied). The fee is transferred from the signer (a self-spend, so no allowance is needed) and disclosed in the signed description. `BASE_FEE_AMOUNT` must be a multiple of `1e13` (5-decimal granularity, so the disclosed amount is exact), and `BASE_FEE_AMOUNT = 0` deploys a feeless macro. The fee configuration is immutable per deployment.
